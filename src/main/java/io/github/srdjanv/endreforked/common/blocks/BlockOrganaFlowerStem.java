...
public class BlockOrganaFlowerStem extends BaseBlockCrops {
    ...
    @Override protected Item getSeed() {
    ...
    public void updateTick(World worldIn, BlockPos plantPos, IBlockState state, Random rand) {
        ...
        if (net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, plantPos, state, rand.nextInt((int) (25.0F / f) + 1) == 0)) {
            if (!isMaxAge(state)) {
                ...
                if (worldIn.isAirBlock(fruitPos)
                    ...
                    IBlockState newState = state.withProperty(AGE, state.getValue(AGE) + 1);
                    if (isDead(newState)) {
                        worldIn.setBlockState(plantPos, ModBlocks.ORGANA_FLOWER_STEM_DEAD_BLOCK.get().getDefaultState(), 2);
            ...
            net.minecraftforge.common.ForgeHooks.onCropsGrowPost(worldIn, fruitPos, state, worldIn.getBlockState(fruitPos));
        }
    }
