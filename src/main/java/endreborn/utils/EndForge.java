...
public final class EndForge {
    ...
    public static boolean hasAction(World world, BlockPos pos, ItemStack stack, @Nullable EnumFacing face) {
        ...
        if (stack.getItem() == ModItems.INGOT_ENDORIUM || stack.getItem() == Items.SLIME_BALL ||
            ...
            return state.getBlock() == ModBlocks.BLOCK_END_FORGE.get();
        }
    ...
    private static boolean handleForgerI(World world, BlockPos pos, EntityPlayer player, ItemStack stack,
                                         EnumHand hand) {
        if (!world.isRemote) {
            EndHelper.dropItemInWorldExact(world, pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5,
                    new ItemStack(ModItems.INFUSED_METALL.get(), 1));
    ...
    private static boolean handleForgerS(World world, BlockPos pos, EntityPlayer player, ItemStack stack,
                                         EnumHand hand) {
        if (!world.isRemote) {
            EndHelper.dropItemInWorldExact(world, pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5,
    ...
    private static boolean handleForgerB(World world, BlockPos pos, EntityPlayer player, ItemStack stack,
                                         EnumHand hand) {
        if (!world.isRemote) {
            EndHelper.dropItemInWorldExact(world, pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5,
    ...
    private static boolean handleForgerE(World world, BlockPos pos, EntityPlayer player, ItemStack stack,
                                         EnumHand hand) {
        if (!world.isRemote) {
            EndHelper.dropItemInWorldExact(world, pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5,
                    new ItemStack(ModBlocks.BLOCK_END_MAGMA.get(), 1));
            player.setHeldItem(hand, EndHelper.consumeItem(player, stack));
            world.playSound(null, pos, SoundEvents.BLOCK_FURNACE_FIRE_CRACKLE, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
        return true;
