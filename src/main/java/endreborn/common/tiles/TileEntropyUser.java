...
import endreborn.common.blocks.BlockEntropyUser;
...
public class TileEntropyUser extends TileEntity implements IInventory, ITickable {
    ...
    public void update() {
        ...
        if (!this.world.isRemote) {
            ...
            if (this.isBurning() || !stack.isEmpty() && !RecipesUser.getInstance()
                    .getAltarResult((ItemStack) this.inventory.get(0), (ItemStack) this.inventory.get(1)).isEmpty()) {
    ...
    private boolean canSmelt() {
        if (RecipesUser.getInstance()
                .getAltarResult((ItemStack) this.inventory.get(0), (ItemStack) this.inventory.get(1)).isEmpty())
            return false;
        else {
            ItemStack result = RecipesUser.getInstance().getAltarResult((ItemStack) this.inventory.get(0),
                    (ItemStack) this.inventory.get(1));
