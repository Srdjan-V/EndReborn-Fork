...
public class ItemEnderBow extends ItemBow implements IHasModel {
    ...
    public ItemEnderBow(String name, float chargeTime, int anim_0, int anim_1, int anim_2) {
        ...
        addPropertyOverride(new ResourceLocation("pull"), (stack, worldIn, entityIn) -> {
            if (entityIn == null)
                ...
                return itemstack != null && itemstack.getItem() == ItemInit.ENDER_BOW ?
                        (stack.getMaxItemUseDuration() - entityIn.getItemInUseCount()) / 20.0F : 0.0F;
            }
        });
        addPropertyOverride(new ResourceLocation("pulling"),
                (stack, worldIn,
    ...
    public ActionResult<ItemStack> onItemRightClick(World par2World, EntityPlayer par3EntityPlayer, EnumHand hand) {
        ...
        ActionResult<ItemStack> ret = ForgeEventFactory.onArrowNock(par1ItemStack, par2World, par3EntityPlayer, hand,
                flag);
        if (ret != null) return ret;

        if (!par3EntityPlayer.capabilities.isCreativeMode && !flag)
            return !flag ? new ActionResult(EnumActionResult.FAIL, par1ItemStack) :
                    new ActionResult(EnumActionResult.PASS, par1ItemStack);
        else {
    ...
    public boolean getIsRepairable(ItemStack par1ItemStack, ItemStack par2ItemStack) {
        return ItemInit.INGOT_ENDORIUM == par2ItemStack.getItem() ? true :
