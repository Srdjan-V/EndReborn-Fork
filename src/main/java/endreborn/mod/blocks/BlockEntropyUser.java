...
public class BlockEntropyUser extends BlockBase {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
        ...
        if (!worldIn.isRemote && !playerIn.isSneaking()) {
            worldIn.playSound((EntityPlayer) null, pos, SoundEvents.BLOCK_LEVER_CLICK, SoundCategory.BLOCKS, 0.5F,
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos) {
        ...
        if (active) {
            ...
            worldIn.playSound((EntityPlayer) null, pos, SoundEvents.BLOCK_PORTAL_TRAVEL, SoundCategory.BLOCKS, 0.5F,
                    2.6F + (worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * 0.8F);
            worldIn.setBlockState(pos, BlockInit.BLOCK_E_USER.getDefaultState()
                    .withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, true), 3);

        } else worldIn.setBlockState(pos, BlockInit.BLOCK_E_USER.getDefaultState()
